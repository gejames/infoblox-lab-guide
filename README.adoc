...
Now that we know how to create a new container in NIOS with Ansible, let's give that ability to a junior admin. They aren't trusted with the keys to the kingdom yet, so we'll put some constraints around them and allow them to create new NIOS containers or network through a Tower survey.

Add a survey to your exsting job.  An example survey is shown below.

image::images\tower-survey.png[]
