...
In order to pass credentials to NIOS, we define a dictionary called `nios_provider`.  In Tower, we pass the NIOS url, username, and password to our playbook from the `nios creds` credentials and `nios` inventory extra vars.
